...
class AccuracyTest {
  ...
  def testRandomForest(): Unit = {
    val baseLearner = new RegressionTreeLearner(numFeatures = nFeat / 3).setHyper("minLeafInstances", minInstances)
  ...
  def testLowTLimit(): Unit = {
    ...
    val errorAnnealingTree = {
      val baseLearner = new RegressionTreeLearner(splitter = new AnnealingSplitter(temperature = 1.0e-9))
      val learner = new Bagger(baseLearner, numBags = nRow * 16)
  ...
  def testLessRandomForest(): Unit = {
    val baseLearner = new RegressionTreeLearner(numFeatures = nFeat).setHyper("minLeafInstances", minInstances)
    val learner = new Bagger(baseLearner, numBags = nRow * nScal)
    val error = computeMetrics(learner)
    assert(error > noiseLevel, s"Can't do better than noise")
  ...
  def testAnnealingForest(): Unit = {
    ...
    val improvements: Seq[(Double, Double)] = (0 until 32).map { i =>
      val baseLearner = new RegressionTreeLearner(splitter = new AnnealingSplitter(temperature = temp)).setHyper("minLeafInstances", minInstances)
      val learner = new Bagger(baseLearner, numBags = nRow * nScal)
      val error = computeMetrics(learner)
...
object AccuracyTest {
  ...
  def computeMetrics(nRow: Int, nFeat: Int, nFeatSub: Int, nScal: Int, minInstances: Int, temperature: Double): (Double, Double) = {
    ...
    val baseLearner = new RegressionTreeLearner(
      ...
      splitter = new AnnealingSplitter(temperature = temperature)
    ).setHyper("minLeafInstances", minInstances)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val nScal = 1
    val minInstances = 1
    val (baseRMSE: Double, baseStdRes: Double) = computeMetrics(nRow, nFeat, nFeat, nScal, minInstances, 1.0e-9)
