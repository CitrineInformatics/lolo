...
class SplitterTest {
  ...
  def testTotalVarianceCalculation(): Unit = {
    ...
    /*
    val totalVarianceSuperShort = leftSquareSum / leftNum - Math.pow(leftSum / leftNum, 2) +
      (totalSquareSum - leftSquareSum)/(totalNum - leftNum) - Math.pow((totalSum - leftSum) / (totalNum - leftNum), 2)
