...
/** Additional regression-specific interface
  *
  * This interface is experimental and SHOULD BE REVIEWED before being merged into `master`. In particular, an
  * explanation of how the different methods relate to each other, how predictive uncertainty is decomposed, and what
  * the assumptions are should be added, as these are currently not entirely clear.
  *
  * For example, does the interface assume that the predictions are the mean of a predictive distribution (as opposed
  * to, for example, the median, or the value with highest probability)? Does it assume the predictive distribution to
  * be normal? Such assumptions are fine, but should be explicitly stated.
  */
trait RegressionResult extends PredictionResult[Double] {
  ...
  /** Get the standard deviation of the distribution of predicted observations, if possible
    *
    * Observations of the predicted variable are expected to have a stddev that matches this value. This statistic is
    * related to the https://en.wikipedia.org/wiki/Prediction_interval It does not include estimated bias, even if the
    * regression result contains a bias estimate.
    */
  def getStdDevObs(): Option[Seq[Double]] = None

  /** Get the expected error of the observations, if possible
    *
    * This statistic is related to the https://en.wikipedia.org/wiki/Prediction_interval This statistic includes the
    * contribution of the estimated bias. E.g., for a normal distribution of predicted means, the total error is
  ...
  /** Get a quantile from the distribution of predicted observations, if possible
    *
    * Observations of the predicted variable are inferred to have a distribution with this quantile. This statistic is
    * related to the https://en.wikipedia.org/wiki/Prediction_interval getObsQuantile(0.5) is a central statistic for
    * the estimated bias, if the bias is estimated but not corrected.
    *
    * @param quantile
  ...
  /** Get the expected error of the predicted mean observations, if possible
    ...
    * that matches this value. This statistic is related to the https://en.wikipedia.org/wiki/Confidence_interval This
    * statistic includes the contribution of the estimated bias. E.g., for a normal distribution of predicted means, the
    * total error is sqrt(bias**2 + variance)
    */
  ...
  /** Get the standard deviation of the distribution of predicted mean observations, if possible
    *
    * The variation is due to the finite size of the training data, which can be thought of as being sampled from some
    * training data distribution. This statistic is related to the variance in the bias-variance trade-off
  ...
  /** **EXPERIMENTAL** Get the estimated bias of each prediction, if possible
    *
    * The bias is signed and can be subtracted from the prediction to improve accuracy. See
    * https://en.wikipedia.org/wiki/Bias%E2%80%93variance_tradeoff
    *
    * It is unclear if this method will be a stable member of the interface. It should be reviewed before the next
    * formal release.
    */
...
trait MultiTaskModelPredictionResult extends PredictionResult[Seq[Any]] {
  ...
  /** Get the correlation coefficients between the predictions made on two labels. Correlation coefficient is bounded
    * between -1 and 1. If either index is out of bounds or does not correspond to a real-valued label, then this method
    * must return None.
    *
    * @param i
