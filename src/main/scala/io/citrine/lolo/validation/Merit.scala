...
/** Square root of the mean square error. For an unbiased estimator, this is equal to the standard deviation of the
...
case object RootMeanSquareError extends Merit[Double] {
  override def evaluate(
    ...
    Math.sqrt(
      (predictionResult.getExpected(), actual).zipped.map { case (x, y) =>
        Math.pow(x - y, 2)
...
/** Measure of the correlation between the predicted uncertainty and error magnitude
  ...
  * predicted uncertainty and the ideal error distribution. That is: let X be the predicted uncertainty and Y := N(0, x)
  * be the ideal error distribution about each predicted uncertainty x. It is the correlation coefficient between X and
  * Y In the absence of a closed form for that coefficient, it is model empirically by drawing from N(0, x) to produce
...
object Merit {
  ...
  def estimateMerits[T](
    ...
    pva
      .flatMap { case (predictions, actual) =>
