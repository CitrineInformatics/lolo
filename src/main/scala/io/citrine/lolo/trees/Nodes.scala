...
trait ModelNode[+T <: PredictionResult[Any]] extends Serializable {
  ...
  /** Compute Shapley feature attributions for a given input in this node's subtree
    ...
    * @return
    *   array of vector-valued attributions for each feature One Vector[Double] per feature, each of length equal to the
    *   output dimension.
    */
  ...
  /** Get the Shapley feature attribution from a subtree
    ...
    *
    * The algorithm recursively tracks the feature that is used for each split and the share of the training data that
    * went to the left and the right of the split. In the leaves, these features and their weights are used to construct
    * the leaf's contribution to the SHAP values, which are then summed.
    *
    * @param input
    *   for which to compute feature attributions.
    * @param featureWeights
    *   Map from feature index to [[FeatureWeightFactor]], which stores the weight of the child of the split when the
    *   feature is known vs unknown
    * @return
...
/** Internal node in the decision tree
  ...
  *   dimension of model output, used for Shapley computation 1 for single-task regression, or equal to the number of
...
class InternalModelNode[T <: PredictionResult[Any]](
  ...
  /** On the way down: append this node to the set of features that have been encountered in this path through the
    * decision tree unless this feature is in the omitted features list.
    *
    * On the way up: sum the contributions from the two children of this node. If the feature that this node splits on
    * is in the omitted features list, then multiply the contributions by the share of the training data that went to
    * each child in lieu of including the feature on the way down.
    */
  def shapleyRecurse(
      input: Vector[AnyVal],
...
class ModelLeaf[T](model: Model[PredictionResult[T]], depth: Int, trainingWeight: Double)
  ...
  /** Compute the contribution to SHAP in the leaf based on the features that were encountered between the root node and
    * this leaf. Note that the order of these features does *not* matter.
    *
    * The contributions are based on a tricky combinatorial factor that can be computed using dynamic programming. For
    * details of this procedure, see [[FeaturePowerSetTerms]].
    */
  def shapleyRecurse(
      input: Vector[AnyVal],
      omitFeatures: Set[Int],
