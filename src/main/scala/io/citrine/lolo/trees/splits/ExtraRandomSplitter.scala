...
/**
  ...
  */
case class ExtraRandomSplitter() extends Splitter[Double] {
  ...
  def getBestRealSplit(
    ...
    val pivot = lowerBound + (upperBound - lowerBound) * Random.nextDouble()
  ...
  def getBestCategoricalSplit(
    ...
    val pivot = Random.nextInt(orderedNames.size - 1)
