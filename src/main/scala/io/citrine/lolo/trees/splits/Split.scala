...
abstract trait Split extends Serializable {
  ...
  /** Take the left branch in the binary split?
    ...
    *   vector of any type
    * @return
    *   true if input takes the left split
    */
  ...
  /** Get the index of the input vector that is used to pick this split
    *
    * @return
    *   index of the input vector used by this split
    */
  def getIndex(): Int
}
...
/** Split based on a real value in the index position
  *
  * @param index
  *   position of the real value to inspect
  * @param pivot
  *   value at or below which to take the left split
  */
