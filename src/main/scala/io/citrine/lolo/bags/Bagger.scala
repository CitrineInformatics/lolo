...
case class Bagger(
  ...
  override def train(trainingData: Seq[(Vector[Any], Any)], weights: Option[Seq[Double]] = None): BaggedTrainingResult[Any] = {
    ...
    // Get bias model and rescale ratio using helper class
    val helper = BaggerHelper(models, trainingData, Nib, useJackknife, uncertaintyCalibration)
    val biasModel = if (biasLearner.isDefined && helper.isRegression) {
      Async.canStop()
    ...
    if (helper.isRegression) {
      new BaggedTrainingResult[Double](models.asInstanceOf[ParSeq[Model[PredictionResult[Double]]]], averageImportance, Nib, trainingData, useJackknife, biasModel, helper.ratio, disableBootstrap)
    } else {
